...
Client.on("guildMemberAdd", async m => {
    if (m.user.bot === true) {
        if (m.guild.id === "264445053596991498") {} else {
            let embed = new Discord.RichEmbed()
                .setDescription(`**Hello, ${m.user}! Welcome to \`${m.guild.name}\`!!! \nPro tip: Type \`${await client.prefixes.get(message.guild.id)}help\` to see a command list!!!**`)
                .setColor('#a200ff')
...
Client.on("message", async message => {
    ...
    if (data) {
      ...
      prefix = cfg.prefix;
      await client.prefixes.set(message.guild.id, cfg.prefix);
    }
